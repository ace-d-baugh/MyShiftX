...
export function buildRoleDropdownItems({
  isAdmin, showModItems, isLeader, showUpgrade = false,
  pendingApprovalsCount, pendingFlagsCount,
}: {
  isAdmin: boolean
  showModItems: boolean
  isLeader: boolean
  /** Basic tier only — adds the "Upgrade to Pro" entry. Off by default so
   *  surfaces that don't know the viewer's tier (e.g. the landing header)
   *  simply omit it. */
  showUpgrade?: boolean
