 ...
 * a hex approximation — this app ships eight themes (see app/globals.css:
 * default, dark, midnight, cyberpunk, nordic, christmas, halloween, kitty,
 * patriotic) with wildly different values for the same token, so a hardcoded
 * hex would only ever match one of them. Same reasoning as the Help page's
 * Legend, which builds its swatches from the real badge classes rather than
 * approximating them.
 */
...
export function buildShiftShareData(
  ...
  // Class strings below are the exact ones components/ui/Badge.tsx maps each
  // variant to — verified against it, so the pills match the real cards.
...
export function buildShareText(data: ShareCardData, url: string): string {
  ...
  lines.push('', 'Found on MyShiftX', url)
  return lines.join('\n')
}
