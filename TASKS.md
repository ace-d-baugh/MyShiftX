...
**Open item, not verified by me:** #30 requires **Manual Linking** enabled in MyShiftX's own Supabase Dashboard → Authentication → Settings, or `linkIdentity()` will fail outright. I have no dashboard access to confirm this from the coding environment — check it before relying on the Connect button. All three items are type-checked, linted, and build clean; not exercised live in-browser (no test credentials in this environment). **Update:** Manual Linking has since been confirmed enabled by the user.

### Port #6: Notifications page + public user profiles (2026-08-31)

The two large items from `PORTABLE_FEATURES_2.md` (29, 32) that were explicitly out of scope for Port #5's "simple things" — ported in full on request.

**#29 Notifications page + Mod/Admin board announcements.** New `notifications`/`notification_recipients` tables with RLS, retrofitted into all 7 existing push call sites (`notifyInterest`, `notifyComment`, `sendMatchNotifications` ×2, `notifyClaimCreated`, `notifyClaimResolved`, `notifyClaimFinalized`, `notifyBoardApproved` in `app/actions/notifications.ts`) via a shared `createNotification()` helper, so the in-app page shows the same content that was pushed. New `app/actions/boardNotifications.ts` lets a Mod (their own boards) or Admin (any board) send a pinned, 14-day board-wide announcement; sender/Admin can edit (resets read state, re-fires push/email) or hard-delete, everyone else gets a per-user Dismiss. New `app/(dashboard)/notifications/` page + client, a Bell entry with unread badge in the account dropdown/Navbar/LandingHeader, and a `purge_expired_notifications()` sweep on `pg_cron` (already enabled on this project) plus opportunistically on page load.

Two deliberate deviations from WDWShiftX's source migration, both to match MyShiftX's own stricter conventions rather than copy verbatim: `get_unread_notification_count()`/`purge_expired_notifications()` have anon `EXECUTE` revoked here (WDW left them at Postgres's default `PUBLIC` grant — MyShiftX's own `get_unread_message_count()` already locks anon out, so this matches precedent); and every new RLS policy uses the `(select auth.uid())` init-plan-optimized form MyShiftX's newer migrations use, instead of WDW's un-wrapped `auth.uid()` — confirmed via the Supabase advisor before and after (`auth_rls_initplan` WARN cleared).

**#32 Public user profiles + names linkified site-wide.** New `bio`/`birthday_month`/`birthday_day`/`birthday_year` columns on `users` (grant added in the same migration, `authenticated` only — public profiles require sign-in, unlike the anon-visible `avatar_url`) and a new `user_contact_methods` table (repeatable phone/email/socials list, owner-write/authenticated-read RLS). New "Public Profile" tab on the Profile page (`PublicProfileEditor.tsx`, `ContactMethodsEditor.tsx`) and a read-only `app/(dashboard)/users/[id]/` page (`PublicProfileClient.tsx`, `ContactMethodRow.tsx`) — a deliberately un-boring hero card, matching WDW's design. New `components/ui/UserLink.tsx` (links to `/profile` for your own name, `/users/[id]` for anyone else's, plain text if no id is available) rolled out to `ShiftCard`, `RequestCard`, `CommentSection`, `TradeRecordSection`, `ClaimSection`, the messages conversation header, `AdminClient`, `AdminLeaderboard`, and `BoardsClient` — the same 10 call sites WDW linkified, each verified to actually have a real user id in scope before wiring it in. New dependency: `react-icons` (brand marks for Instagram/Facebook/X/TikTok/Discord/Snapchat/LinkedIn in `lib/contactMethods.ts`; Phone/Email keep `lucide-react`).

One deviation from the source: the contact-method placeholder examples used WDW's `@dorkface` in-joke handle — replaced with a neutral `@yourhandle` rather than presuming that voice fits MyShiftX. Visibility policy kept as WDW built it (any signed-in user can view anyone's public profile) per the doc's own note that a `SECURITY DEFINER` RPC would be needed instead for row-level scoping — not implemented here, since MyShiftX has no board-scoping requirement for this yet.

Migrations applied live via Supabase MCP: `notifications`, `lock_down_notification_functions`, `public_profile_fields`, `optimize_notification_and_contact_rls`. Advisor checked clean (0 ERROR-level findings) before and after. Type-check, lint, and full production build all clean throughout — not exercised live in-browser (no test credentials in this environment); the Notifications page's send/edit/delete flow and the public profile editor's save round-trip are the parts most worth a `/verify` pass.
