...
export default async function AdminPage() {
  ...
  const [boardsRes, usersRes, memberRowsRes, postStatsRes] = await Promise.all([
    supabase
      ...
      .order('name')
      .limit(200),
    // Membership/billing_cycle/first_name/last_name are column-locked from
    // direct SELECT for the authenticated role — this RPC is internally
    // gated to Admins only and returns them for every user.
